 ...
 * ./.wp-env.json, creates ~/.wp-pod-env, ~/.wp-pod-env/docker-compose.yml, and
 * ~/.wp-pod-env/Dockerfile.
 *
...
module.exports = async function initConfig( {
	...
	// @todo This isn't great but it's the best I am willing to commit to until I get the rest of this package working with podman.
	// A more ideal approach would be to specify the ports like ":3306" where the host port is left empty rather than omitted entirely.
	// However `podman-compose port mysql 3306` will blow up on us - `podman-compose port` is only capable of reading docker-compose.yml.
	// We would have to rework the usage of `podmanCompose.port()` in start.js to call `podman port` directly instead of podman-compose.
	// @see https://github.com/containers/podman-compose/issues/778
	const mysqlPort = await getPort();
	dockerComposeConfig.services.mysql.ports = [`${mysqlPort}:3306`];

	const testsMysqlPort = await getPort();
	dockerComposeConfig.services['tests-mysql'].ports = [`${testsMysqlPort}:3306`];
	...
	if ( writeChanges ) {
		...
		// Write two WordPress Dockerfiles, one for development environment and one for test environment.
...
function wordpressDockerFileContents( env, config ) {
	...
	return `FROM docker.io/library/wordpress${ phpVersion }
...
function installDependencies( service, env, config ) {
	...
	// @todo Come back and ditch the switch now that we no longer have cli services?
	switch ( service ) {
		case 'wordpress': {
			dockerFileContent += `
...
RUN apt-get -qy install git default-mysql-client
...
RUN composer global require --dev wp-cli/wp-cli-bundle:"^2.0"
USER root`;
...
RUN echo 'xdebug.client_host="host.containers.internal"' >> /usr/local/etc/php/php.ini`;
}
