...
async function checkDatabaseConnection( { dockerComposeConfigPath, debug } ) {
	await podmanCompose.run( 'wordpress', 'wp db check', {
		config: dockerComposeConfigPath,
		commandOptions: [ '--rm', '--user', hostUser.fullUser ],
		log: debug,
	} );
}
...
async function resetDatabase(
	...
	if ( environment === 'all' || environment === 'development' ) {
		tasks.push( podmanCompose.run( 'wordpress', 'wp db reset --yes', options ) );
	}

	if ( environment === 'all' || environment === 'tests' ) {
		tasks.push(
			podmanCompose.run( 'tests-wordpress', 'wp db reset --yes', options )
		);
	}
