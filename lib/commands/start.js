...
module.exports = async function start( {
	...
	if ( shouldConfigureWp ) {
		...
		try {
			...
			await exec( `podman volume rm ${ volumesToRemove }` );
		} catch {
		...
		await podmanCompose.pullAll( dockerComposeConfig );
		spinner.text = 'Downloading sources.';
	}

	await Promise.all( [
		podmanCompose.upOne( 'mysql', {
			...dockerComposeConfig,
	...
	if ( shouldConfigureWp ) {
		await podmanCompose.buildOne( [ 'cli' ], { ...dockerComposeConfig } );
	}
	...
	const { stdout: mySQLAddress } = await exec(
		`podman port $(podman ps -a --format {{.Names}} --filter label=io.podman.compose.project=$(basename ${ workDirectoryPath }) --filter label=com.docker.compose.service=mysql) 3306`
	);
	const mySQLPort = mySQLAddress.trim().split( ':' ).pop();

	const { stdout: testsMySQLAddress } = await exec(
		`podman port $(podman ps -a --format {{.Names}} --filter label=io.podman.compose.project=$(basename ${ workDirectoryPath }) --filter label=com.docker.compose.service=tests-mysql) 3306`
	);
	const testsMySQLPort = testsMySQLAddress.trim().split( ':' ).pop();
