...
function spawnCommandDirectly( config, container, command, envCwd, spinner ) {
	...
	// @todo Here for back compat with wp-env but maybe we can drop these altogether?
	if ( container === 'cli' || container === 'tests-cli' ) {
		container = container.replace( 'cli', 'wordpress' );
	}
