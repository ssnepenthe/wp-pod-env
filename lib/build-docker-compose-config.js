...
function getMounts(
	...
	const directoryMounts = Object.entries( config.mappings ).map(
		( [ wpDir, source ] ) => `${ source.path }:/var/www/html/${ wpDir }:z`
	);

	const pluginMounts = config.pluginSources.map(
		( source ) =>
			`${ source.path }:/var/www/html/wp-content/plugins/${ source.basename }:z`
	);

	const themeMounts = config.themeSources.map(
		( source ) =>
			`${ source.path }:/var/www/html/wp-content/themes/${ source.basename }:z`
	);
...
module.exports = function buildDockerComposeConfig( config ) {
	...
	if (
		...
		testsMounts.unshift(
			...[
				`${ wpSource.testsPath }:/var/www/html:z`,
				...( wpSource.type === 'local'
					? fs
